...
export function CombinedLoanSimulator({
  ...
  const defaultLump = isInr ? '50000' : '5000'
  const defaultRecurring = isInr ? '15000' : '200'
  const recurringLabel = isInr ? '+₹15k/mo' : '+$200/mo'
  const [events, setEvents] = useState<SimEvent[]>(() => [
    newEvent({ type: 'one_time_prepayment', amount: defaultLump, label: 'Bonus prepay' }),
    newEvent({
      type: 'recurring_prepayment',
      amount: defaultRecurring,
      months: '36',
  ...
  const [commitAmount, setCommitAmount] = useState(defaultRecurring)
          ...
          {result && (
            <div className="space-y-4 pt-2">
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <Kpi label="Interest saved" value={formatCurrency(result.kpis.interest_saved, currency, locale)} accent />
                <Kpi label="Months saved" value={String(result.kpis.months_saved)} />
                <Kpi label="EMI" value={formatCurrency(result.kpis.emi, currency, locale)} />
                ...
                <div key={c.id} className="flex items-center justify-between border rounded-lg px-3 py-2 text-sm">
                  <div>
                    <div className="font-medium">
                      {c.kind === 'recurring_prepayment' ? 'Recurring' : 'One-time'} {formatCurrency(c.amount, currency, locale)}
                    </div>

    ...
    <div className="space-y-2">
      {data.map((row) => (
        <div key={row.period} className="flex flex-wrap gap-4 border rounded-md p-3 text-sm">
          ...
          <span>Principal {formatCurrency(row.principal_component, currency, locale)}</span>
          <span>Interest {formatCurrency(row.interest_component, currency, locale)}</span>
          <span>Total {formatCurrency(row.total_paid, currency, locale)}</span>
          {row.prepayments > 0 && <span>Prepay {formatCurrency(row.prepayments, currency, locale)}</span>}
        </div>
      ))}
    </div>
  )
